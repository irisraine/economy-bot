...
def yahtzee():
    embed_message = MessageContainer(
        ...
        description=f"Добро пожаловать в покер на костях — игру, где прихотливые законы вероятностей и стратегия идут "
                    ...
                    f"- **Фулл-хаус** (пара и тройка с одинаковыми значениями): **x1.5**\n"
                    f"- **Большой стрит** (последовательность из пяти чисел): **x2**\n"
                    f"- **Каре** (четыре кости с одинаковым значением): **x5**\n"
    ...
    if income < 0:
        ...
        if roll_outcome['winning_combination'] in winning_type['tiny']:
            ...
            description = (f"Кубики прыгали по столу, будто бы решая твою судьбу. Сердце замерло, когда они "
                           ...
                           f"глазам: это и впрямь {winning_name[roll_outcome['winning_combination']]}! Выигранные "

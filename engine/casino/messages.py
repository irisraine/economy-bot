...
from engine.config import FROG_EMOJI, BASIC_COLOR_CODE, ERROR_COLOR_CODE, ERROR_IMAGE
...
def slot_machine():
    embed_message = MessageContainer(
        ...
        description="Испытай свою удачу в игре с одноруким бандитом и сорви свой джекпот! Выбирай между двумя режимами "
                    ...
                    f"- **Жабий чвяк** (стоимость игры **5** {FROG_EMOJI})\n"
...
def slot_machine_result(player, reels, bet, payout, image_binary_data):
    ...
    if payout == 0:
        ...
        result = (f"К сожалению, символы центральной линии не сложились ни в одну из выигрышных комбинаций! "
        ...
        result = (f"<@{player.id}>, ты не веришь собственным глазам, однако это правда: символы центральной линии "
...
def roulette():
    embed_message = MessageContainer(
        ...
        description="Европейская рулетка — это классическая азартная игра, где удача решает всё! На игровом поле есть "
                    ...
                    "или нечётные числа, ставки на низкие или высокие числа, а ещё можно выбирать дюжины, ряды и "
                    "сикслайны (пары двух соседних колонок). Каждая ставка имеет свой коэффициент выигрыша, так что "
                    "выбирай стратегию мудро!\n"
                    ...
                    f"- Ставка на число: от **1** {FROG_EMOJI} до **10** {FROG_EMOJI}\n"
                    f"- Ставка на диапазон: от **5** {FROG_EMOJI} до **25** {FROG_EMOJI}\n\n"
                    ...
                    "*Выплаты в зависимости от типа ставки:*\n\n"
                    "- Одно число: **35:1**\n"
...
def roulette_result(player, sector, overall_bet, winnings):
    bet_categories = {
        "straight": "Число",
        "color": "Красное/черное",
        "even_odd": "Чет/нечет",
    ...
    if sector['color'] == "green":
        sector_description = "0 (зеро) 🟢"
    else:
        if sector['color'] == "red":
            sector_description = f"{sector['number']} (красное) 🔴"
        elif sector['color'] == "black":
            sector_description = f"{sector['number']} (черное) ⚫️"
    ...
    embed_message = MessageContainer(
        ...
        description="Колесо рулетки замерло, и в зале повисла тишина, нарушаемая только мерным ночным кваканьем "
                    ...
                    f"🏆 ***Выпавший сектор***: __**{sector_description}**__\n"
                    f"🎟 ***Общая сумма всех твоих ставок***: **{overall_bet}** {FROG_EMOJI}\n"
...
def yahtzee():
    embed_message = MessageContainer(
        ...
        description="Добро пожаловать в захватывающий мир 'Покера на костях' — игру, где удача и стратегия идут "
                    ...
                    "- **Покер** (все пять костей одинаковы): **20:1**\n\n"
                    f"Минимальный размер ставки: **3** {FROG_EMOJI}, максимальный: **15** {FROG_EMOJI}\n\n"
...
def wrong_player_error():
    embed_message = MessageContainer(
        title=ERROR_HEADER,
        description="Ковбой, это не твоя игра! Ее начал другой игрок, и только он имеет доступ к панели игры. "
                    "Если хочешь сыграть в казино, используй команду `/casino` и выбери для себя игру самостоятельно!",

...
def slot_machine():
    embed_message = MessageContainer(
        ...
        description=f"Испытай свою удачу в игре с одноруким бандитом и сорви свой джекпот! Выбирай между двумя "
                    ...
                    f"{config.SLOT_MACHINE_REEL_SYMBOLS['frog_white']['emoji']}** "
                    f" - выплата "
                    f"**{config.SLOT_MACHINE_PAYOUT_AMOUNTS['frogs_all_colors']}** {FROG_EMOJI}\n"
                    f"**{config.SLOT_MACHINE_REEL_SYMBOLS['frog_green']['emoji']} "
                    f"{config.SLOT_MACHINE_REEL_SYMBOLS['frog_green']['emoji']} "
                    f"{config.SLOT_MACHINE_REEL_SYMBOLS['frog_green']['emoji']}** "
                    f" - выплата "
...
def roulette_result(player, sector, overall_bet, winnings, image_binary_data):
    ...
    if income > 0:
        ...
        title = "Увы и ах, сегодня тебе не повезло!"
        if income < 0:
            description += (f"К сожалению, не всегда удача улыбается игрокам. \n"
...
def yahtzee():
    embed_message = MessageContainer(
        ...
        description=f"Добро пожаловать в захватывающий мир покера на костях — игру, где прихотливые законы "
                    ...
                    f"Правила игры просты. Сделай ставку и брось пять кубиков, после чего посмотри, что выпадет. "
                    f"В случае, если ты сразу собрал одну из выигрышных комбинаций — поздравляем, ты победитель! "
    ...
    if income < 0:
        ...
        if roll_outcome['winning_combination'] in winning_type['small']:
            description = (f"Кости с глухим стуком ударились о стол и замерли, показав скромную выигрышную комбинацию:"
                           ...
                           f"начале игры, — и задумался: продолжить или уйти? Ведь вечер ещё не кончился, а удача всё "
        ...
        if roll_outcome['winning_combination'] in winning_type['large']:
            description = (f"Кубики прыгали по столу, будто бы решая твою судьбу. Сердце замерло, когда они "
                           ...
                           f"вечера. Сегодня удача была твоей спутницей, и весь Лаграс казался готовым склониться "

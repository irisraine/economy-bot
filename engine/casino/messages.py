...
def slot_machine_result(player, reels, bet, payout, image_binary_data):
    ...
    if payout == 0:
        ...
        description += (f"На центральной линии не сложилось ни одной выигрышных комбинаций - тем не менее, благодаря "
...
def roulette():
    embed_message = MessageContainer(
        title="**Рулетка**",
        description=f"В рулетке участники делают ставки на то, куда упадёт шарик, брошенный на вращающееся колесо. "
...
def yahtzee():
    embed_message = MessageContainer(
        ...
        description=f"Добро пожаловать в покер на костях — игру, где прихотливые законы вероятностей и стратегия идут "
                    ...
                    f"- **Малый стрит** (последовательность из четырех чисел): **0.5:1**\n"
                    f"- **Фулл-хаус** (пара и тройка с одинаковыми значениями): **1:1**\n"
                    f"- **Каре** (четыре кости с одинаковым значением): **2:1**\n"
                    f"- **Большой стрит** (последовательность из пяти чисел): **5:1**\n"
                    f"- **Покер** (все пять костей одинаковы): **10:1**\n\n"
...
def yahtzee_result(player, bet, payout, roll_outcome, image_binary_data, is_reroll=False):
    winning_type = {
        'small': ["small_straight", "full_house", "four_of_a_kind"],
...
def balance_error(is_fraud=True):
    ...
    if is_fraud:
        description += ("На твоем счету недостаточно средств для игры. Параллельно с текущей игрой ты играл в другую "

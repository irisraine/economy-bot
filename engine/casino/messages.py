...
def slot_machine_result(player, reels, bet, payout, image_binary_data):
    ...
    if payout == 0:
        ...
        description += (f"<@{player.id}>, к твоей радости, символы на центральной сложились в выигрышную комбинацию: "
                        ...
                        f"Эта скромная победа приятно согревает душу — пускай это не джекпот, но всё же удача "
                        f"улыбнулась тебе.")
    else:
    ...
    if income < 0:
        ...
        if not is_reroll:
            ...
            description += (f"**<@{player.id}>, ты проиграл {bet} {FROG_EMOJI}.\n**"
                            f"Ты медленно выдохнул, отпуская надежду на удачу. Карман опустел, но азарт всё ещё пульсировал в "
                            f"груди. Со стола раздался чей-то смешок — игра продолжалась, но для тебя она уже закончилась."
                            f"\n\nУдачи в следующей игре! 🍀")
    else:

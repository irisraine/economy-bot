...
@catch_sql_exceptions
def create_user_balance(user_discord_id):
    with sqlite3.connect("database/vault.db") as db_connect:
        cursor = db_connect.cursor()
        cursor.execute("INSERT INTO user_balances (user_discord_id) VALUES (?)",
                       (user_discord_id,))
...
def get_all_users_balances():
    with sqlite3.connect("database/vault.db") as db_connect:
        cursor = db_connect.cursor()
        cursor.execute("SELECT user_discord_id, balance FROM user_balances WHERE balance > 0 ORDER BY balance DESC")
...
def get_bank_balance():
    with sqlite3.connect("database/vault.db") as db_connect:
        cursor = db_connect.cursor()
        cursor.execute("SELECT balance FROM user_balances")
        return cursor.fetchone()[0]
...
def set_bank_balance(amount):
    with sqlite3.connect("database/vault.db") as db_connect:
        cursor = db_connect.cursor()
        cursor.execute("UPDATE balance SET balance = balance + ?",
                       (amount,))
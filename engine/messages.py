...
def member_removed_from_server(removed_user, user_balance):
    embed_message = MessageContainer(
        ...
        description=f"Пользователь {removed_user.mention} покинул наш сервер. Поскольку его больше нет с нами, и "
                    f"вскорости время сотрет его имя, то принадлежавшее ему болото вместе со всеми пойманными "
                    f"лягушками в количестве **{user_balance}** {config.FROG_EMOJI} отныне переходит в собственность "
                    f"администрации.",
